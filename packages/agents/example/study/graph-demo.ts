...
async function demo1_helloWorld() {
  ...
  type SimpleState = typeof SimpleState.State
  ...
  async function greetingNode(state: SimpleState) {
    const userInput = state.input
    return { output: `你好！你输入的是："${userInput}"` }
    // 只更新 output 字段，input 保持不变
  }

  async function responseNode(state: SimpleState) {
...
async function demo2_chatbot() {
  ...
  type ChatState = typeof ChatState.State
  // ChatState = { messages: BaseMessage[] }

  // --- Step 2: 定义节点 ---

  async function chatbotNode(state: ChatState) {
    // 从 state 中取出消息历史
...
async function demo3_reactAgent() {
  ...
  const toolsByName = { [add.name]: add, [multiply.name]: multiply } as const
  ...
  type ReactState = typeof ReactState.State

  // --- Step 3: 定义节点 ---

  async function llmNode(state: ReactState) {
  ...
  async function toolNode(state: ReactState) {
    const lastMsg = state.messages[state.messages.length - 1] as AIMessage
    const results: ToolMessage[] = await Promise.all(
      (lastMsg.tool_calls ?? []).map((tc) => {
        const tool = toolsByName[tc.name as keyof typeof toolsByName]
  ...
  function shouldContinue(state: ReactState): "tools" | typeof END {
    const lastMsg = state.messages[state.messages.length - 1] as AIMessage
...
async function demo4_conditionalRouting() {
  const RouterState = Annotation.Root({
    content: Annotation<string>({ reducer: (_) => _, default: () => "" }),
    category: Annotation<string>({ reducer: (_) => _, default: () => "" }),
    result: Annotation<string>({ reducer: (_) => _, default: () => "" }),
  })
  type RouterState = typeof RouterState.State

  // --- 节点 ---

  async function categorizeNode(state: RouterState) {
    const len = state.content.length
    let category: string
    if (len < 10) category = "short"
    else if (len < 30) category = "medium"
    else category = "long"
    return { content: state.content, category }
  }

  async function handleShort(state: RouterState) {
    return { result: `短文本(${state.category}): "${state.content}" — 快速处理` }
  }

  async function handleMedium(state: RouterState) {
    return { result: `中等文本(${state.category}): "${state.content}" — 正常分析` }
  }

  async function handleLong(state: RouterState) {
  ...
  function routeByCategory(state: RouterState): "handle_short" | "handle_medium" | "handle_long" {
...
async function demo5_parallelExecution() {
  const ParallelState = Annotation.Root({
    topic: Annotation<string>({ reducer: (_) => _, default: () => "" }),
    sentiment: Annotation<string>({ reducer: (_) => _, default: () => "" }),
    summary: Annotation<string>({ reducer: (_) => _, default: () => "" }),
    translation: Annotation<string>({ reducer: (_) => _, default: () => "" }),
    result: Annotation<string>({ reducer: (_) => _, default: () => "" }),
  })
  type ParallelState = typeof ParallelState.State

  // --- 节点 ---

  async function analyzeSentiment(state: ParallelState) {
  ...
  async function combineResults(state: ParallelState) {
    return {
      result: [
        `原文: ${state.topic}`,
        `情感: ${state.sentiment}`,
...
async function demo6_humanInTheLoop() {
  const HtlState = Annotation.Root({
    ...
    approval: Annotation<"approved" | "rejected" | null>({
      reducer: (_) => _,
      default: () => null,
    }),
  })
  type HtlState = typeof HtlState.State

  async function draftNode(state: HtlState) {
  ...
  async function reviewNode(state: HtlState) {
  ...
  function needsReview(state: HtlState): "review" | "draft" | typeof END {
    const lastMsg = state.messages[state.messages.length - 1] as AIMessage
    if ((lastMsg.tool_calls?.length ?? 0) > 0) return END
    const approval = state.approval
    if (approval === null) return "review"
    if (approval === "rejected") return "draft"
    return END
  }
  ...
  const result1 = await graph.invoke(
    { messages: [new HumanMessage("帮我写一句产品宣传语")] },
    config,
  )
  console.log("Draft 节点输出:")
  console.log((result1.messages[result1.messages.length - 1] as AIMessage).content)
  ...
  const result2 = await graph.invoke(
    ...
    config,
    // 在 Human-in-the-loop 场景中，传入更新后的 State + 原始 config 即可恢复执行
    // LangGraph 会自动检测到这是同一个 thread_id 的续跑请求
  )
  console.log("最终输出:")
  for (const msg of result2.messages) {
    if (msg.getType() === "ai") console.log(msg.content)
  }
