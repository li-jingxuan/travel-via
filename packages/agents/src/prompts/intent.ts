...
3. 对于 destination 和 departurePoint：如果用户没说，不要猜测，输出空字符串 ""
4. 如果用户使用了近义表达或非标准说法，请根据语义选择最接近的一项枚举，不要输出枚举之外的值
5. 只输出纯 JSON，不要有 markdown 包裹或额外文字
6. 示例数据不能作为输入、输出（目的地和出发地等必要信息必须要求用户明确输入，否则留空）

示例输入："我想去新疆自驾游，大概15天，6月份去"
示例输出：
{"destination":"新疆","departurePoint":"","days":15,"month":"6月","travelType":"自驾"}

示例输入："带父母去云南玩一周"
示例输出：
{"destination":"云南","departurePoint":"","days":7,"month":"未指定","travelType":"自由行","travelers":"父母"}`
